...
class MetricAlarm:
    ...
    #         'OKActions': self.get('OKActions'),
    #         'InsufficientDataActions': self.get('InsufficientDataActions'),
    ...
    def to_dict(self):
        for key, value in self.metric_alarm.items():
            if isinstance(value, datetime.datetime):
                self.metric_alarm[key] = str(value)
...
def handler(event, context):
    for alarm in list_alarms():
        TABLE.put_item(Item=MetricAlarm(alarm).to_dict())


CLOUDWATCH = boto3.client("cloudwatch")
PAGINATED_LIST_OF_ALARMS = CLOUDWATCH.get_paginator("describe_alarms")
TABLE = boto3.resource(
    "dynamodb", endpoint_url=f"https://dynamodb.{region()}.amazonaws.com"
